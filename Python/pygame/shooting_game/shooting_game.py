...
def main():
    ...
    badguy = BadguySprite(game_images.get('badguy'), position=(64, 100))
    badguy_sprites_group.add(badguy)
    # 定义计时器
    badtimer = 100
    badtimer1 = 0
    # 循环主体
    runnning, exitcode = True, False
    ...
    while running:
        ...
        # -- 倒计时信息
        countdown_text = font.render(str((90000-pygame.time.get_ticks())//60000)+":"+\
        str((90000-pygame.time.get_ticks())//1000%60).zfill(2), True, (0, 0, 0))
        countdown_rect = countdown_text.rect()
        ...
        for event in pyame.event.get():
            if event.type == pygame.QUIT:
                ...
                arrow = ArrowSprite(game_images.get('arrow'), (angle, bunny.rotated_positon[0]+32), bunny.rotated_position[1]+26)
        ...
        # -- 画出兔子
        bunny.draw(screen, pygame.mouse.get_pose())
        # -- 画出健康值，先画了红色的生命值条，后根据根据数值添加绿色
